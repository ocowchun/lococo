...
Dispatcher.register(function(action) {
  ...
  handles[EVENTS.ADD_WORD] = function() {
    ...
    const dictionary = FileStore.getCurrentDictionary();
    const newDictionary = dictionary.addValue(word, defination, locale);

    FileStore.emit(EVENTS.ADD_WORD, newDictionary.listValueLocales(word));
  }
  ...
  handles[EVENTS.SAVE_DICTIONARY] = function() {
    const dictionary    = FileStore.getCurrentDictionary();
    
    dictionary.save().then(function() {
      FileStore.emit(EVENTS.SAVE_DICTIONARY);
    });
  }
